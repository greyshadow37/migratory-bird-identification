...
MODEL_DIR = "./model/efficientnetb0"
PLOT_DIR = "./plots/efficientnetb0"
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(targets)), targets), 1):
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            train_bar = tqdm(train_loader, desc=f"Fold {fold} Epoch {epoch} [Train]", leave=True)
            ...
            for imgs, labels in train_bar:
                ...
                train_bar.set_postfix({'Loss': running_loss / len(all_train_labels)})
            ...
            val_bar = tqdm(val_loader, desc=f"Fold {fold} Epoch {epoch} [Val]", leave=True)
            ...
            with torch.no_grad():
                for imgs, labels in val_bar:
                    ...
                    val_bar.set_postfix({'Loss': val_loss / len(all_labels)})
    ...
    summary_text = (
        f"Final Results across {K} folds:\n"
