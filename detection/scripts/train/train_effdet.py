...
class Detection(CocoDetection):
    def __init__(self, root, annFile, img_size=512):
...
def plot_confusion_matrix(matrix, class_names, plots_path):
    ...
    try:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            norm = matrix[:, :-1] / (matrix[:, :-1].sum(axis=1, keepdims=True) + 1e-12)
        df_norm = pd.DataFrame(norm, index=class_names, columns=class_names)
        plt.figure(figsize=(14, 12))
        sns.heatmap(df_norm, annot=False, cmap='Greens', vmin=0, vmax=1)
        plt.title('EfficientDet - Confusion Matrix (Normalized)')
        plt.xlabel('Predicted Label'); plt.ylabel('True Label')
        plt.savefig(os.path.join(plots_path, 'confusion_matrix_normalized.png'), dpi=300, bbox_inches='tight')
        plt.close()
    except Exception as e:
...
def plot_precision_recall_curves(precisions, recalls, f1s, confidences, plots_path):
    ...
    try:
        ...
        plt.figure(figsize=(10, 8))
        plt.plot(confidences, precisions, linewidth=2)
        plt.title('EfficientDet - Precision vs. Confidence Curve')
        plt.xlabel('Confidence'); plt.ylabel('Precision'); plt.grid(True)
        plt.savefig(os.path.join(plots_path, 'precision_confidence_curve.png'), dpi=300)
        plt.close()

        # Recall-Confidence Curve
        plt.figure(figsize=(10, 8))
        plt.plot(confidences, recalls, linewidth=2)
        plt.title('EfficientDet - Recall vs. Confidence Curve')
        plt.xlabel('Confidence'); plt.ylabel('Recall'); plt.grid(True)
        plt.savefig(os.path.join(plots_path, 'recall_confidence_curve.png'), dpi=300)
        plt.close()
        try:
            fpr = (1 - precisions)
            tpr = recalls
            ...
            plt.plot(fpr, tpr, label='ROC Approx')
            plt.plot([0,1],[0,1],'--',color='gray')
            plt.xlabel('False Positive Rate'); plt.ylabel('True Positive Rate'); plt.title('Approximate ROC Curve')
            plt.grid(True)
...
def train_model(args, model, train_loader, val_loader):
    ...
    scaler = torch.amp.GradScaler(enabled=(args.half and device.type == 'cuda'))
    ...
    for epoch in range(args.epochs):
        ...
        for images, targets in tqdm(train_loader, desc=f"Epoch {epoch+1}/{args.epochs}"):
            ...
            optimizer.zero_grad()
            with torch.amp.autocast(enabled=(args.half and device.type == 'cuda')):
...
def main():
    ...
    try:
        ...
        val_dataset = Detection(root=args.data_path, annFile=args.ann_file.replace('train', 'val'), img_size=args.img_size) # Assumes a val set
